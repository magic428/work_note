...
className = ["Lump_coal", "Shearer_drum", "Coal_miner", "Face_sprag", "Abnormal_alignment", "Abnormal_alignment"]
colors = [(255, 0, 0), (0, 255, 0), (100, 0, 180), (255, 180, 0), (180, 0, 100), (150, 120, 255)]
...
def saveXML(root_path, name, objs, cls, w, h):
    global pre_labeled_file_name
    with open(os.sep.join([root_path, 'Annotations', name])+'.xml', "w") as xml:
    ...
    pre_labeled_file_name = os.sep.join([root_path, 'Txts', os.path.splitext(name)[0] + ".txt"])

    print('pre_labeled_file_name: ', pre_labeled_file_name)
    with open(os.sep.join([root_path, 'Txts', os.path.splitext(name)[0]])  + ".txt", "w") as txt:
        txt.write("%d,%d\n" % (len(objs), cls))
        for item in objs:
            d = item[0]
            e = item[1]
...
def sort_as_filename(imgs):
    ...
    for i in range(len(imgs)):
        ...
        imgs[i] = "{}{:>06d}{}".format(file_name_pre,imgs[i],file_name_post)
        # print(imgs[i])
...
def do_label(imgs):
    ...
    while True:
        ...
        pos = imgs[i][0].rfind(".")
        print(imgs[i][0][:pos])
        currentClass, objs = loadObjs("%s/%s/%s.txt" %(path, 'Txts', imgs[i][0][:pos]))
        waitSecDown = False
        ...
        while True:
            ...
            if key == ord('s'): #s
                ...
                saveXML(path, imgs[i][0][:pos], objs, currentClass, show.shape[1], show.shape[0])
                break
...
if __name__ == "__main__":
    ...
    imgs = os.listdir(os.sep.join([path, 'JPEGImages']))
    print('1', imgs[:3])
    sort_as_filename(imgs)
    ...
    imgs = [(imgs[i], os.sep.join([path, 'JPEGImages', imgs[i]])) for i in range(len(imgs))]
